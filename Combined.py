...
def execute_trial(algo_func, dataset, target_diff, epsilon, algo_name, trial, factor_idx, size_idx, size, **kwargs):
    """
    Executes a single trial of an algorithm and returns the results.
    Accepts additional keyword arguments for specific algorithms.
    """
    start_time = time.time()
    result, removals, iterations, differences = algo_func(dataset.copy(), target_diff, epsilon, **kwargs)
    elapsed_time = time.time() - start_time
    final_diff = calc_avg(result)

    # Determine if a solution was found
    if final_diff is not None and target_diff - epsilon <= final_diff <= target_diff + epsilon:
        solution_found = True
    else:
...
def log_factor_averages(size, size_results, factor_idx, size_stats_file, epsilon_factor):
    ...
    Only includes trials where a solution was found.
    """
    ...
    for algo_name, metrics in size_results.items():
        ...
        if total_trials > 0:
            ...
            avg_removals = np.mean(removals) if removals else 0
        ...
        for algo_name, metrics in size_results.items():
            ...
            for i in range(current_size_idx + 1):
                ...
                if values:
                    ...
                    avg = float('nan')  # Use NaN for no data
...
def initialize_size_results(epsilon_adjust_factors, database_sizes):
    """
    Initializes the results data structure for storing removals, time metrics, and successes for each algorithm.
    """
    return {
        algo: {
        ...
        with open(os.path.join(size_folder, "size_stats.txt"), "w") as size_stats_file:
            ...
            for factor_idx, epsilon_factor in enumerate(epsilon_adjust_factors):
                ...
                for trial in range(1, num_trials + 1):
                    ...
                    # Adjust target difference based on target percentage
                ...
                for res in results:
                    ...
                    if res['solution_found']:
                        ...
                        size_results[algo]["successes"][factor_idx][res['size']]["values"].append(True)
                    else:
                        # Log the failure with the correct trial number
...
def main():
    ...
    epsilon_adjust_factors = [1000, 10000, 100000]
